...
- `lib/lead-api.ts` (`server-only`) is the only API caller; `app/page.tsx` turns an API error into `ApiErrorCard` instead of crashing. `components/leads/leads-panel.tsx` polls with `router.refresh()` every 5s while leads are `pending`.
